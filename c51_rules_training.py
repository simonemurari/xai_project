...
def _plot_pmfs(
    pmfs_rules, pmfs, combined_pmfs, action_index, n_categories, title_prefix
):
    """
    ...
    print(f'ACTION: {action_index}, rule_pmf: {rule_pmf}, network_pmf: {network_pmf}, combined_pmf: {combined_pmf}, rule_pmf_shape: {rule_pmf.shape}, network_pmf_shape: {network_pmf.shape}, combined_pmf_shape: {combined_pmf.shape}')
    # Create subplots
    fig, axs = plt.subplots(1, 3, figsize=(15, 5))

    # Plot the rule-based PMF
    axs[0].bar(x, rule_pmf, label="Rule PMF", color="skyblue")
    axs[0].set_title(f"{title_prefix} Rule PMF - Action {action_index}")
    axs[0].set_xlim(0, 1)
    ...
    axs[1].set_title(f"{title_prefix} Network PMF - Action {action_index}")
    axs[1].set_xlim(0, 1)
    ...
    axs[2].set_title(f"{title_prefix} Combined PMF - Action {action_index}")
    axs[2].set_xlim(0, 1)
    ...
    plt.tight_layout()
    if not os.path.exists("images_rules_training/"):
        os.makedirs("images_rules_training/")
    plt.savefig(f"images_rules_training/{title_prefix}_pmfs_{action_index}.png")
...
class QNetwork(nn.Module):
    ...
    def get_action(self, x, action=None, global_step=0, total_timesteps=Args.total_timesteps * 0.5):
        ...
        combined_values = alpha * q_values + (1 - alpha) * weights
        if action is None:
...
if __name__ == "__main__":
    ...
    if args.save_model:
        ...
        plt.ylabel("Return")
        plt.grid(True)
        plt.savefig(f"{args.env_id}_c51rt_{args.total_timesteps}_{start_datetime}_eval.png")
