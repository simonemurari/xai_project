...
# Rules applied to the C51 algorithm only during exploration while training
...
if __name__ == "__main__":
    ...
    run_name = f"C51rexp_{args.env_id}__seed{args.seed}__{start_datetime}"
    if args.track:
        import wandb
        wandb.tensorboard.patch(root_logdir=f"C51rexp/runs_rules_training/{run_name}/train")
        wandb.init(
    ...
    writer = SummaryWriter(f"C51rexp/runs_rules_training/{run_name}/train")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s"
    ...
    for global_step in tqdm(range(args.total_timesteps), colour="green"):
        ...
        if random.random() < epsilon:
            weights = q_network.get_suggested_action(get_observables(obs))
    ...
    plt.title(f'C51rexp on {args.env_id} - Return over {args.total_timesteps} timesteps')
    plt.xlabel("Episode")
    plt.ylabel("Return")
    plt.grid(True)
    path = f'C51rexp/{args.env_id}_c51rexp_{args.total_timesteps}_{start_datetime}'
    if not os.path.exists("C51rexp/"):
        os.makedirs("C51rexp/")
    os.makedirs(path)
    plt.savefig(f"{path}/{args.env_id}_c51rexp_{args.total_timesteps}_{start_datetime}.png")
    plt.close()
    with open(f"{path}/c51rexp_args.txt", "w") as f:
    ...
    if args.save_model:
        model_path = f"{path}/c51rexp_model.pt"
        model_data = {
        ...
        writer = SummaryWriter(f"C51rexp/runs_rules_training/{run_name}/eval")
        for idx, episodic_return in enumerate(episodic_returns):
            writer.add_scalar("episodic_return", episodic_return, idx)

        plt.plot(episodic_returns)
        plt.title(f'C51rexp Eval on {args.env_id} - Return over {eval_episodes} episodes')
        plt.xlabel("Episode")
        plt.ylabel("Return")
        plt.ylim(0, 1)
        plt.grid(True)
        plt.savefig(f"{path}/{args.env_id}_c51rexp_{eval_episodes}_{start_datetime}_eval.png")
        ...
        if args.upload_model:
            ...
            push_to_hub(args, episodic_returns, repo_id, "C51rexp", f"runs/{run_name}", f"videos/{run_name}-eval")
