...
class QNetwork(nn.Module):
    ...
    def get_rule_mask(self, batch_of_observables):
        """
        Highly optimized version with vectorized operations and minimal Python overhead
        """
        device = self.atoms.device  # Use same device as network
        batch_size = len(batch_of_observables)

        # Pre-allocate tensors
        rule_mask = torch.zeros(batch_size, self.n, device=device)

        # Process batch in parallel with vectorized operations
        for batch_idx, observables in enumerate(batch_of_observables):
            # Quickly extract all relevant data using dict lookups instead of loops
            ...
            door_states = {}  # Map door colors to states

            # Optimized data extraction - single pass through observations
            for name, args in observables:
                # Fast path processing using if/elif instead of match/case for older Python versions
                # and potentially better performance
                if name == "door" and args[1:] == [0, 1]:
                    has_door_front = True
                    door_color = args[0]
                elif name == "key" and args[1:] == [0, 1]:
                    has_key_front = True
                    key_color = args[0]
                    ...
                    door_states[args[0]] = name

            # Rule 1: pickup(X) :- key(X), samecolor(X,Y), door(Y), notcarrying
            if not carrying_key and has_key_front and key_color:
                # Optimized check for matching door color
                door_with_matching_color = any(
                    name == "door" and args[0] == key_color
                    for name, args in observables
                )

                if door_with_matching_color:
                    rule_mask[batch_idx, 3] = 1

            # Rule 2: open(X) :- door(X), locked(X), key(Z), carryingKey(Z), samecolor(X,Z)
            if has_door_front and carrying_key and door_color == carrying_key:
                # Fast check if door is locked
                if door_color in door_states and door_states[door_color] == "locked":
                    rule_mask[batch_idx, 5] = 1

            # Rule 3: goto :- goal(X), unlocked
            # Optimized path checking logic
            if goal_data[1]:  # Goal is directly in front
                # Check for blocking obstacles
                front_blocked = wall_state[2] or (
                    has_door_front
                    and door_color in door_states
                    and door_states[door_color] == "locked"
                )
                if not front_blocked:
                    rule_mask[batch_idx, 2] = 1
            elif goal_data[0]:  # Goal has an x offset
                if goal_data[0] < 0 and not wall_state[0]:  # Left
                    rule_mask[batch_idx, 0] = 1
                elif goal_data[0] > 0 and not wall_state[1]:  # Right
                    rule_mask[batch_idx, 1] = 1
                else:  # Try moving forward if not blocked
                    front_blocked = wall_state[2] or (
                        has_door_front
                        and door_color in door_states
                        and door_states[door_color] == "locked"
...
def train_c51_rules(args, seed, process_idx, num_processes):
    ...
    if args.track:
        ...
        wandb.init(
            ...
            monitor_gym=True,
            save_code=True,
            group=f"C51rtv3_{0.75}_{25}_{args.exploration_fraction}_mp_{args.run_code}",
        )
