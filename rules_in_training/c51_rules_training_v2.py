...
if __name__ == "__main__":
    ...
    run_name = f"C51rtv2_{args.env_id}__seed{args.seed}__{start_datetime}"
    if args.track:
        import wandb
    ...
    plt.title(f'C51rtv2 on {args.env_id} - Return over {args.total_timesteps} timesteps')
    plt.xlabel("Episode")
    plt.ylabel("Return")
    plt.grid(True)
    path = f'C51rtv2/{args.env_id}_c51rtv2_{args.total_timesteps}_{start_datetime}'
    if not os.path.exists("C51rtv2/"):
        os.makedirs("C51rtv2/")
    os.makedirs(path)
    plt.savefig(f"{path}/{args.env_id}_c51rtv2_{args.total_timesteps}_{start_datetime}.png")
    plt.close()
