...
if __name__ == "__main__":
    ...
    if args.track:
        ...
        wandb.init(
            ...
            monitor_gym=True,
            save_code=True,
            group=f"C51rtv3_{args.exploration_fraction}_sum_eps_EXP_ONLY_{args.run_code}",
        )
    ...
    for global_step in progress_bar:
        ...
        if random.random() < epsilon:
            ...
            actions, pmf = q_network.get_action(
                torch.Tensor(obs).float().to(device), skip=True
            )
            # Get rule suggestions based on the current observation
            rule_actions = q_network._apply_rules_batch(
