...
class QNetwork(nn.Module):
    ...
    def _navigate_towards(self, target_x, target_y, walls=None):
        """
        Navigation helper that avoids walls when moving towards a target
        Returns action index instead of action name
        """
        ...
        if target_y > 0:  # Target is in front
            if not wall_in_front:
                ...
                return 0 if target_x <= 0 else 1  # left or right
        elif target_x < 0:  # Target is to the left
            return 0  # left
        elif target_x > 0:  # Target is to the right
            return 1  # right
            ...
            return 1 if wall_to_left else 0  # right or left
...
def train_c51(args, seed):
    ...
    for global_step in progress_bar:
        ...
        if random.random() < epsilon:
            ...
            weights = q_network.get_suggested_action(obs_img).squeeze().cpu().numpy()
