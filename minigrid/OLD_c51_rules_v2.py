...
def _plot_pmfs(
    ...
    plt.tight_layout()
    if not os.path.exists("AAAplots/"):
        os.makedirs("AAAplots/")
    plt.savefig(f"AAAplots/{title_prefix}_pmfs_{action_index}_{alpha:.2f}_{datetime.now().strftime('%Y_%m_%d-%H_%M_%S')}.png")
    plt.close()
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    bias = - epsilon * 0.8 * args.n_atoms
                    next_atoms = data.rewards + args.gamma * target_network.atoms * (
                        1 - data.dones
                    )
                    print(f'0_next_atoms= {next_atoms}, shape={next_atoms.shape}\n')
                    next_atoms += rule_mask.float().unsqueeze(1) * bias
                    print(f'1_next_atoms= {next_atoms}, shape={next_atoms.shape}\n')

                    # projection
                    delta_z = target_network.atoms[1] - target_network.atoms[0]
                    tz = next_atoms.clamp(args.v_min, args.v_max)
                    print(f'tz= {tz}, shape={tz.shape}\n')
                    ...
                    d_m_u = (b - l) * next_pmfs
                    print(f'd_m_l= {d_m_l}, shape={d_m_l.shape}\n')
                    print(f'd_m_u= {d_m_u}, shape={d_m_u.shape}\n')
                    target_pmfs = torch.zeros_like(next_pmfs)
                    for i in range(target_pmfs.size(0)):
                        target_pmfs[i].index_add_(0, l[i].long(), d_m_l[i])
                        target_pmfs[i].index_add_(0, u[i].long(), d_m_u[i])
                    print(f'target_pmfs= {target_pmfs}, shape={target_pmfs.shape}\n')
                    _plot_pmfs(
                        target_pmfs.cpu(),
                        next_pmfs.cpu(),
                        target_pmfs.cpu(),
                        data.actions.flatten()[0].item(),
                        args.n_atoms,
                        epsilon,
                        "Target",
                    )
