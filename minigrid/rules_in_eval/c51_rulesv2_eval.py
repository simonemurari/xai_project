...
from rules_in_training.c51_rules_training_v2 import make_env, QNetwork
from matplotlib import pyplot as plt
from datetime import datetime

# Evaluation of a C51 model on a MiniGrid environment, rules from C51_rules_training_v2.py applied only during evaluation
...
def evaluate(
    ...
    model.eval()
    writer = SummaryWriter(f"C51rev2/runs_eval/{run_name}/eval")
    writer.add_text(
        "hyperparameters",
    ...
    for i in range(eval_episodes):
        if random.random() < epsilon:
            rule_mask = model.get_suggested_action(get_observables(obs))
            weights = rule_mask * 0.8 + (1 - rule_mask) * 0.2
            weights = weights / weights.sum(1, keepdim=True)
            weights = weights.cpu().numpy()[0]
            actions = np.random.choice(range(model.n), p=weights)
            actions = np.array([actions])
        else:
            actions, _ = model.get_action(torch.Tensor(obs).to(device), global_step=i, total_timesteps=eval_episodes * .75)
...
if __name__ == "__main__":
    ...
    if os.path.exists(f"C51rev2/{path}") is False:
        os.makedirs(f"C51rev2/{path}")
    run_name = f"C51rev2/{path}/{Args.env_id}_c51rev2_{Args.total_timesteps}_{start_datetime}"
    args = tyro.cli(Args)
    run_name = f"C51rev2_trainedon={path.split('_')[0]}__testedon={args.env_id}__seed={args.seed}__{start_datetime}"
    eval_episodes = 10000
    if args.track:
        import wandb
        wandb.tensorboard.patch(root_logdir=f"C51rev2/runs_eval/{run_name}/eval")
        wandb.init(
    ...
    plt.grid(True)
    plt.savefig(
        f"C51rev2/{path}/{args.env_id}_c51rev2_{eval_episodes}_{start_datetime}.png"
    )
